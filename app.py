...
def generate_image(prompt):
    """Photorealistic image via Pollinations Flux."""
    import random
    ...
    html = (
        ...
        "onerror=\"this.parentElement.innerHTML='&#10060; Could not generate image.'\">"
...
IMAGE GENERATION:
You can generate and edit images. When the user asks for a new image OR wants to edit/redo/change a previously generated image, you MUST reply with ONLY this on a single line — no other text before or after:
##IMAGE:your detailed prompt here##
Rules:
- Write a rich descriptive prompt (lighting, style, details, mood).
- For edits/redos, check chat history for the last image and apply the user's requested changes.
- Phrases like "redo it", "try again", "make it X instead", "change to", "now make it" are image edit requests.
- Output ONLY ##IMAGE:prompt## — nothing else at all for image requests.
...
def _build_reply(user_msg):
    ...
    if session["awaiting_owner_code"]:
        ...
        if "##OWNER_CLAIM##" in raw_reply:
            ...
            # Extract prompt between ##IMAGE: and ## (flexible — works even if Groq adds extra text)
            import re as _re
            ...
            if m:
                ...
                # Fallback: grab everything after ##IMAGE:
