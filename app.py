...
app.config["MAX_COOKIE_SIZE"] = 4093  # Explicit Flask cookie size limit
...
# Secret slug for the admin panel URL — set ADMIN_SLUG in your env vars
# e.g. ADMIN_SLUG=x7k2mq9p  →  access via /admin/x7k2mq9p
ADMIN_SLUG     = os.environ.get("ADMIN_SLUG", "x7k2mq9p")
ADMIN_BASE     = f"/admin/{ADMIN_SLUG}"
IST            = timezone(timedelta(hours=5, minutes=30))
DATABASE_URL   = os.environ["DATABASE_URL"]

# Admin session stays alive for 7 days
...
def _auto_analyse_generated_image(img_prompt, session_messages, img_src=None, username=None, chat_key=None):
    ...
    Appends the result to session_messages (a COPY — not the live session list),
    then persists it to the DB chat session so it survives across requests.
    ...
    try:
        ...
        if description:
            ...
            # Append to the local copy so the brain can use it on this request
            session_messages.append(analysis_entry)
            # Persist to DB so it survives into the next request
            if username and chat_key:
                try:
                    save_chat_session(username, chat_key, session_messages)
                except Exception:
                    pass
    except Exception:
...
def _compact_for_session(text):
    """Replace bulky content with compact placeholders before storing in session.
    Keeps the session cookie well under Flask's 4 KB limit.
    The full HTML is returned to the frontend separately — the session only
    needs a text representation so the brain can understand context."""
    # Generated image HTML  →  short caption placeholder
...
def _build_clean_history(history):
    ...
    for msg in history[-30:]:
        ...
        # ── Generated image — compact placeholder (new sessions) ──
        ...
        # ── Generated image — legacy raw HTML blob (old sessions) ──
        ...
        # ── Edited image — compact placeholder (new sessions) ──
        ...
        # ── Edited image — legacy raw marker (old sessions) ──
...
def _build_reply(user_msg):
    ...
    if action == "text":
        ...
        if "##IMAGE:" in raw_reply:
            ...
            threading.Thread(
                ...
                args=(img_prompt, list(session["messages"]), None,
                      session.get("user"), session.get("chat_key")),
                daemon=True
            ).start()
            return reply
    ...
    if action in ("generate_image", "edit_image"):
        ...
        # Pass a COPY of messages (thread-safe) and username+chat_key so
        # the result gets persisted to DB and is available on the next request.
        _msgs_copy = list(session["messages"])
        threading.Thread(
            target=_auto_analyse_generated_image,
            args=(img_prompt, _msgs_copy, _img_src,
                  session.get("user"), session.get("chat_key")),
            daemon=True
        ).start()
        # Return the full HTML to the caller (goes to frontend),
        # but the session will store a compact placeholder (applied in send()).
        return reply
...
def send():
    ...
    # Store a compact version in session (prevents cookie overflow).
    # The full reply (with HTML, images, etc.) goes only to the frontend.
...
def send_image():
    ...
    try:
        ...
        session.setdefault("messages", [])
        session["messages"].append({"sender": "You", "text": user_label})
        # Store a compact version in session; full reply (with base64) only to frontend.
        if intent == "analyse":
...
def share_chat():
    ...
    for m in msgs:
        txt = m.get("text", "")
        # Skip internal analysis markers — not useful for readers
        if txt.startswith("[IMAGE ANALYSIS RESULT]"):
            continue
        # Convert compact image placeholder to a readable note for shared view
        if txt.startswith("[IMAGE GENERATED:"):
...
# ══════════════════════════════════════════════════════════════════
#  ADMIN SECTION  (/admin/<slug>/*)
# ══════════════════════════════════════════════════════════════════

def _admin_urls():
    """Helper: return the URLs to pass into templates."""
    return {
...
def _admin_check():
    """Return 403 JSON if not logged in as admin."""
    import json
...
def admin_stats():
    """Lightweight endpoint polled every 10s to refresh dashboard numbers."""
    import json
    err = _admin_check()
    if err: return err
    conn = get_db(); cur = conn.cursor(cursor_factory=RealDictCursor)
...
# ── Admin: AI chat analysis (profile-only — no raw chat content exposed) ──

@app.route(ADMIN_BASE + "/analyse_chats/<path:username>", methods=["POST"])
def admin_analyse_chats(username):
    """Use Groq to produce an AI summary/review using stored profile data only.
    Raw chat messages are never read or returned."""
    import json
    err = _admin_check()
    if err: return err
    ...
    profile = get_profile(username)
    if not profile:
        return json.dumps({"error": "No profile data found for this user"}), 404, {"Content-Type": "application/json"}
    ...
    if not msg_count:
        return json.dumps({"error": "No usage data found for this user"}), 404, {"Content-Type": "application/json"}
...
Format:
{
  ...
  "recommendation": "1 sentence recommendation"
    ...
    usr_p = (
        f"Username: {username}\n"
        f"Total user messages: {msg_count} | Avg message length: {avg_len} chars\n"
        f"Top topics (keyword-extracted): {top_topics}\n"
        f"Current sentiment tag: {sentiment}\n"
        f"Previous summary: {groq_summary}\n"
    )
