...
def contains_bad_words(text):
    lower = text.lower()
    return any(re.search(r'\b' + re.escape(word) + r'\b', lower) for word in BAD_WORDS)
...
def send_image():
    ...
    try:
        ...
        save_chat(session["user"], "You", user_label, sid)
        # Never store raw base64/data-URI in session — store a display placeholder instead
        session_reply = "[Edited image generated]" if reply.startswith("__EDITED_IMAGE__") else reply
        save_chat(session["user"], "Jarvis", "[Edited image generated]" if reply.startswith("__EDITED_IMAGE__") else reply, sid)
        session.setdefault("messages", [])
        session["messages"].append({"sender": "You",   "text": user_label})
        session["messages"].append({"sender": "Jarvis","text": session_reply})
        session.modified = True
